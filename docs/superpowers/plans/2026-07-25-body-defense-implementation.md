...
| D25 | `applyPoison` runs once per enemy in range, so a crowd poisons proportionally. Emergent from a loop, never chosen — raised as review finding C1. | **Keep it, and state it.** Same shape as D10's clot wear, in the same step loop; two case rules pricing a crowd differently would be the real inconsistency. Kept on measurement, not principle: repricing it flat moves stomach 5.5% → 6.5%, past throat's 6.3%, which inverts the season's curve and fails the sweep's monotonic assertion. Capping buys nothing — across the whole board space a cell is inside `POISON_RADIUS` on 3.7% of cell-steps, mean crowd 1.16, largest ever 4. No brief copy, unlike D10: every clot in stomach dies to wear so the crowd is its whole story, which is not true here. Tested at the call site in `movement.test.ts`, not on `applyPoison` — the stacking is a property of the caller. |
